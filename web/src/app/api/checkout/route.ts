  ...
  try {
    ...
    if (!body?.items?.length) {
      return NextResponse.json(
        { ok: false, error: { code: "EMPTY_CART", message: "Giỏ hàng trống" } },
        { status: 400 }
      );
    }
    if (!body.customerName || !body.customerAddress) {
      return NextResponse.json(
        { ok: false, error: { code: "BAD_PAYLOAD", message: "Thiếu thông tin khách hàng" } },
        { status: 400 }
      );
    }

    // Tra cứu sản phẩm từ MongoDB
    ...
    const totals = calcTotals(
      snapshot.map((s) => ({ price: s.price, quantity: s.quantity })),
    ...
    if (msg.startsWith("OUT_OF_STOCK:")) {
      return NextResponse.json(
        { ok: false, error: { code: "OUT_OF_STOCK", message: msg.replace("OUT_OF_STOCK:", "") } },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: { code: "INTERNAL", message: "Checkout failed" } },
      { status: 500 }
    );
  }
}
