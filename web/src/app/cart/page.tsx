...
export default function CartPage() {
  ...
  return (
    <>
      {/* ===================== CART PAGE ===================== */}
      <main className="py-8 px-4 max-w-6xl mx-auto min-h-screen">
        <h1 className="text-2xl font-semibold">Giỏ hàng</h1>

        {items.length === 0 ? (
          <div className="mt-6 text-gray-600">
            Giỏ hàng của bạn đang bị trống.
            <Link className="underline ml-1" href="/shop">
              Mua sắm ngay →
            </Link>
          </div>
        ) : (
          <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              {items.map((it) => (
                <div
                  key={it.productId}
                  className="flex gap-4 border rounded-xl p-3 bg-white shadow-sm"
                >
                  <Image
